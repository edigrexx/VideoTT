...
| `PIXABAY_API_KEY` | Необязательный ключ Pixabay: вторая библиотека, которая ищется вместе с Pexels |
...
Другие ограничения: источники за paywall/защитой и PDF не обходятся; stock — иллюстративный видеоряд, не гарантированно точные исторические кадры; crop иногда требует ручной оценки. Стоковый поиск никогда не отвечает «не найдено»: он возвращает полную страницу слабо связанных клипов, поэтому приложение оценивает каждый результат по его собственному описанию и отбрасывает то, что не пересекается с запросом. Для каждой сцены модель даёт точный и запасной ключевой запрос; сначала берётся полное совпадение по точному запросу, затем по запасному, затем частичное, и лишь потом повтор клипа. Если ничего не подходит, сцена падает с `NO_RELEVANT_STOCK`, а не подставляет случайный кадр. Текущая озвучка генерируется по сценам, поэтому на стыках возможны паузы. Edge TTS — community package, работающий с онлайн-сервисом Microsoft, без SLA; для устойчивого коммерческого использования можно заменить TTSProvider. Лицензии исходников нужно соблюдать, включая ограничения на использование узнаваемых людей/брендов. Нет музыки, фронтенд-дашборда, автоматической публикации, account management или мониторинга соцсетей.
