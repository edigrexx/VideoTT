...
class OpenAIProvider:
    ...
    async def generate_script(self, research):
        ...
        script = await self.structured(
            ...
            "Keep narration natural, concrete, engaging and non-repetitive. The hook is the very first sentence, "
            "4–8 words, and must state one surprising concrete fact that opens a gap the rest of the video closes. "
            "Do not phrase the hook as a question and do not restate the title. For a video titled 'How vinyl "
            "records store sound', 'How do vinyl records store our sound?' is wrong because it only asks the title "
            "back; 'The groove holds no bits at all' is right because it states something the viewer wants "
            "explained. End with a satisfying one-sentence payoff of about 8–12 words. "
            ...
            "facts in the last. Give every scene two stock-footage search queries. They MUST be written in "
            "English with Latin letters even when the narration is in another language, because the stock "
            "libraries only index English. These go to a "
